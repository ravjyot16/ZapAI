...
function WriteArticles() {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Write a detailed, well-structured article about ${input}. The article should be ${selectedLength.text} words long. Use proper Markdown formatting with a clear title, headings, subheadings, paragraphs, and bullet points where appropriate.`;
  ...
  return (
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      >
        ...
        >
          {loading ? (
            <span className="w-4 h-4 my-4 rounded-full border-2 border-t-transparent animate-spin"></span>
          ) : (
            <Edit className="w-5" />
          )}
          Generate article
